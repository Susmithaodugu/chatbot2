...
class CustomCSVLoader(CSVLoader):
    def __init__(self, file_path: str, columns_to_tokenize: List[str], **kwargs):
    ...
    def load(self) -> List[Document]:
        ...
        for doc in documents:
            for col in self.columns_to_tokenize:
                if col in doc.metadata and doc.metadata[col] is not None:
                    doc.metadata[col] = self.tokenize_text(doc.metadata[col])
...
LOADER_MAPPING = {
    ".csv": (CustomCSVLoader, {"columns_to_tokenize": ["TicketNbr","Urgency","status_description","service_location","ServiceType","company_name","summary"]}),
    # ".csv": (CSVLoader, {}),
    # ".docx": (Docx2txtLoader, {}),
...
def main():
    ...
    if does_vectorstore_exist(persist_directory):
        ...
        # Split the texts into batches before adding to the vectorstore
        batch_size = 41666
        ...
        # Split the texts into batches before creating the vectorstore
        batch_size = 41666
        for i in range(0, len(texts), batch_size):
            batch_texts = texts[i:i + batch_size]
